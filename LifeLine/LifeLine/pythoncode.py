...
def requested_item():
    ...
    print(lid)
    q="SELECT `item`.`Item_name`,`user`.`First_Name`,`user`.`Last_Name`,`item`.`Quantity`,`request`.`Request_id` FROM request JOIN item ON `request`.`Item_id`=`item`.`Item_id` JOIN `user` ON user.`Login_id`=`request`.`Login_id` WHERE `request`.`Item_id`=%s  AND `request`.`Donor_id`=%s"
    valll=(type,str(lid))
    l=selectall(q,valll)
    qry = "SELECT * FROM item"
    qs = select(qry)
...
#         print(login)
#         q="SELECT `deliveryagent`.`First_Name`,`deliveryagent`.`Last_Name`,`deliveryagent`.`Login_id`,`item`.*,`user`.`First_Name`,`user`.`Last_Name`,`user`.`Login_id`,`assign`.`Status`,`assign`.`Assign_id` FROM assign JOIN `deliveryagent` ON `deliveryagent`.`Login_id`=`assign`.`Agent_id` JOIN `request` ON `request`.`Request_id`=`assign`.`Request_id` JOIN `user` ON `user`.`Login_id`=`request`.`Login_id` JOIN `item`  ON `item`.`Item_id`=`request`.`Item_id` WHERE `request`.`Donor_id`=%s"
#         val=str(login)
#         p=selectall(q,val)
#
#         return  render_template("Add_Report.html",val=p)
